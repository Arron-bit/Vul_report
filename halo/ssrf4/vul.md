# The endpoint `/apis/uc.api.storage.halo.run/v1alpha1/themes/{name}/upgrade-from-uri` of Halo is vulnerable to SSRF attacks
...
1. Use the endpoint `/apis/uc.api.storage.halo.run/v1alpha1/themes/{name}/upgrade-from-uri` to send a request to an attacker-controlled server ([`http://172.22.128.2:5000`](http://172.22.128.2:5000/)).
