# The endpoint `/apis/uc.api.storage.halo.run/v1alpha1/themes/-/install-from-uri` of Halo is vulnerable to SSRF attacks
...
1. Use the endpoint `/apis/uc.api.storage.halo.run/v1alpha1/themes/-/install-from-uri` to send a request to an attacker-controlled server ([`http://172.22.128.2:5000`](http://172.22.128.2:5000/)).
...
1. **Unvalidated URI Input**: The `UpgradeFromUriRequest` accepts user-controlled URI input without any validation or sanitization.
...
3. **No Protocol Restriction**: The implementation does not restrict the URI scheme, allowing protocols beyond HTTP/HTTPS.
